              ...
              <div class="w-full px-6 sm:w-1/2 xl:w-1/3">
                <div class="flex items-center px-5 py-6 bg-white rounded-md shadow-sm">
                  ...
                  <div class="mx-3">
                    ...
                    <button @click="goto_dash" class="text-2xl font-semibold text-gray-700">{{count_user}}</button>
...
export default {
  ...
  created() {
    ...
    axios.post(url + 'api/count_task_for_user_assign/'+ this.username).then((resp) => {
      if (resp.status === 200) {
        localStorage.setItem('count_task_for_user_assign', resp["data"]["data"]);
        console.log(resp);
      }
    }).catch((e) => {
      console.log(e);
